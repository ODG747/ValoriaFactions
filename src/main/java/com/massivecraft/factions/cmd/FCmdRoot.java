...
import com.massivecraft.factions.util.ValoriaModules;
...
public class FCmdRoot extends FCommand implements CommandExecutor {
    ...
    public FCmdRoot() {
        ...
        if (ValoriaModules.enabled("upgrades")) this.addSubCommand(this.cmdUpgrades);
        this.addSubCommand(this.cmdColeader);
        //this.addSubCommand(this.cmdBanner);
        //this.addSubCommand(this.cmdTpBanner);
        if (ValoriaModules.enabled("legacy-tools")) this.addSubCommand(this.cmdKillHolograms);
        ...
        if (ValoriaModules.enabled("faction-chest")) this.addSubCommand(this.cmdChest);
        this.addSubCommand(this.cmdCorner);
        this.addSubCommand(this.cmdCornerList);
        if (ValoriaModules.enabled("legacy-tools")) this.addSubCommand(this.cmdFGlobal);
        if (ValoriaModules.enabled("faction-chest")) this.addSubCommand(this.cmdViewChest);
        this.addSubCommand(this.cmdSpawnerLock);
        if (ValoriaModules.enabled("drain")) this.addSubCommand(this.cmdDrain);
        if (ValoriaModules.enabled("legacy-tools")) this.addSubCommand(this.cmdLookup);
        ...
        if (ValoriaModules.enabled("discord")) {
    ...
    public void addVariableCommands() {
        ...
        //PayPal
        if (ValoriaModules.enabled("paypal") && FactionsPlugin.getInstance().getConfig().getBoolean("fpaypal.Enabled", false) && !fPayPalEnabled) {
        ...
        if (ValoriaModules.enabled("missions") && FactionsPlugin.getInstance().getFileManager().getMissions().getConfig().getBoolean("Missions-Enabled", false) && !missionsEnabled) {
            this.addSubCommand(this.cmdMissions);
            missionsEnabled = true;
        }
        if (ValoriaModules.enabled("inventory-see") && FactionsPlugin.getInstance().getConfig().getBoolean("f-inventory-see.Enabled", false) && !invSeeEnabled) {
            this.addSubCommand(this.cmdInventorySee);
            invSeeEnabled = true;
        }
        if (ValoriaModules.enabled("points") && FactionsPlugin.getInstance().getConfig().getBoolean("f-points.Enabled", false) && !fPointsEnabled) {
