...
public class TimerManager implements Listener, Runnable {
    ...
    public TimerManager(FactionsPlugin plugin) {
        ...
        long updateTicks = Math.max(4L, plugin.getConfig().getLong("performance.timer-update-ticks", 20L));
        plugin.getServer().getScheduler().runTaskTimer(plugin, this, updateTicks, updateTicks);
    }
